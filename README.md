...
* **Manual Assets (`POST /api/manual-accounts`, `PUT /api/manual-accounts/:accountId/balance`)**: Adds owner-entered savings, retirement, or investment balances without consuming a Plaid Item. Current state lives under `users/{uid}/manual_accounts`; every update also writes an exact account snapshot and the existing per-day `users/{uid}/balance_snapshots/{YYYY-MM-DD}` history. The Accounts response combines these with linked balances, marks manual values explicitly, and excludes identity matches from all totals to prevent double-counting.
