...
import { LayoutDashboard, ReceiptText, Wallet, Settings, Menu, X, RefreshCcw, Loader2 } from 'lucide-react';
...
export function AppShell({ children, syncing, onSync, activeTab, setActiveTab }: AppShellProps) {
        ...
        <nav className="flex-1 px-4 py-4 space-y-1">
          {NAV_ITEMS.map(item => (
            ...
            >
              ...
              {item.disabled && <span className="ml-auto text-[10px] uppercase tracking-wider text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full">Soon</span>}
            </button>
          ))}
        </nav>
      </aside>
      ...
      <main className="flex-1 md:ml-64 flex flex-col min-h-screen w-full relative">
        ...
        <header className="md:hidden bg-white border-b border-slate-200 sticky top-0 z-30 pt-[env(safe-area-inset-top)]">
          <div className="px-4 h-16 flex items-center justify-between">
            ...
            >
              {syncing ? <Loader2 className="w-5 h-5 animate-spin text-indigo-600" /> : <RefreshCcw className="w-5 h-5" />}
            </button>
          </div>
        </header>
        ...
        <header className="hidden md:flex bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-10 h-16 items-center justify-between px-8">
          ...
          >
            {syncing ? <Loader2 className="w-4 h-4 animate-spin text-indigo-600" /> : <RefreshCcw className="w-4 h-4" />}
