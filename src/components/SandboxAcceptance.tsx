...
export function SandboxAcceptance({ user }: { user: any }) {
  ...
  const triggerRefresh = async () => {
    ...
    try {
      const headers = { Authorization: `Bearer ${await user.getIdToken()}` };
      const res = await fetch('/api/dev/sandbox-refresh', { method: 'POST', headers });
      ...
      <div className="flex justify-between items-start mb-6">
        ...
        <div className="flex gap-2">
          <button 
            ...
            disabled={refreshing}
            className="px-4 py-2 bg-amber-100 text-amber-800 rounded font-bold hover:bg-amber-200 disabled:opacity-50 text-sm"
          >
          ...
          <button 
            ...
            disabled={loading}
            className="px-4 py-2 bg-slate-800 text-white rounded font-bold hover:bg-slate-700 disabled:opacity-50 text-sm"
          >
