...
// Add import
const importStatement = "import { buildConnectedAccounts } from './server/lib/accounts';\n";
content = content.replace('import { deduplicateAndNormalizeTransactions', importStatement + 'import { deduplicateAndNormalizeTransactions');

const commentAccounts = `// Ledger account inventory.
...
content = content.replace('app.get("/api/accounts"', commentAccounts);

const newEndpoint = `app.get("/api/accounts", requireAuth, async (req: express.Request, res: express.Response) => {
...
content = content.replace('app.get("/api/accounts", requireAuth, async (req: express.Request, res: express.Response) => {', newEndpoint);
