...
describe('Sandbox Acceptance Logic', () => {
  it('exact pending->posted ID matching requires Removed At and proper status/pending flags', () => {
    const rawPending = buildRawRow({
      txId: 'old_1',
      pending: 'TRUE',
      cashFlowAmount: '-50',
      status: 'removed',
      removedAt: '2026-08-15'
    });

    const rawPosted = buildRawRow({
      txId: 'new_1',
      pending: 'FALSE',
      cashFlowAmount: '-50',
      pendingTransactionId: 'old_1'
    });
  ...
  it('explicit cash withdrawal returns PASS', () => {
    const rawWithdrawal = buildRawRow({
      txId: 'w_1',
      cashFlowAmount: '-100',
      catPrimary: 'TRANSFER_OUT',
      catDetailed: 'TRANSFER_OUT_WITHDRAWAL'
    });
  ...
  it('ATM fee does NOT exercise cash-withdrawal scenario', () => {
    const rawAtmFee = buildRawRow({
      txId: 'w_1',
      name: 'ATM FEE',
      cashFlowAmount: '-5',
      catDetailed: 'BANK_FEES_ATM_FEE'
    });
  ...
  it('actual other positive returns PASS for ambiguous positive', () => {
    const rawOtherPos = buildRawRow({
      txId: 'op_1',
      cashFlowAmount: '50',
      catDetailed: 'OTHER_MISCELLANEOUS'
    });
  ...
  it('interest_earned does NOT exercise ambiguous-positive scenario', () => {
    const rawInterest = buildRawRow({
      txId: 'ie_1',
      cashFlowAmount: '5',
      catDetailed: 'INCOME_INTEREST_EARNED',
      name: 'INTEREST'
    });
...
describe('Sandbox Acceptance - Removed Supersessions', () => {
  ...
  it('an actually independent removed transaction can still cause removedReversed = PASS', () => {
    ...
    const indRemoved = buildRawRow({
      txId: 'ind_1',
      pending: 'No',
      cashFlowAmount: '-50',
      status: 'removed',
      removedAt: '2026-08-16'
    });
  ...
  it('excludes multiple pending supersessions from removedReversed', () => {
    const rows: any[] = [['Transaction ID']];
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      rows.push(
        buildRawRow({
          ...
          pending: 'No',
          cashFlowAmount: '-50',
          pendingTransactionId: `old_${i}`,
          status: 'active'
        })
      );
    }

    const report = generateAcceptanceReport(rows);

    expect(report.scenarios.pendingToPosted.status).toBe('PASS');
    expect(report.scenarios.removedReversed.status).toBe('NOT EXERCISED');
  });

  it('excludes multiple pending supersessions but still detects one independent removal', () => {
    const rows: any[] = [['Transaction ID']];
...
describe('Sandbox Acceptance - Posted Acceptance Buckets', () => {
  it('excludes pending credit-card-payment transactions from the Acceptance Credit Card Payment posted count', () => {
    ...
    const rawPendingCC = buildRawRow({
      txId: 'pend_1',
      name: 'AUTOMATIC PAYMENT - THANK',
      cashFlowAmount: '-50',
      accountType: 'credit',
  ...
  it('Credit Card Payment Acceptance matches reconciliation', () => {
    ...
    expect(report.scenarios.creditCardPayment.count)
      .toBe(report.reconciliation.creditCardCount);

    expect(report.scenarios.creditCardPayment.amount)
      .toBe(report.reconciliation.creditCardAmount);
  });

  it('Merchant Credit Acceptance count/amount equal the authoritative posted reconciliation values', () => {
    const rawPurchase = buildRawRow({
      txId: 'p1',
    ...
    expect(report.scenarios.merchantCredit.count)
      .toBe(report.reconciliation.merchantCreditsCount);

    expect(report.scenarios.merchantCredit.amount)
      .toBe(report.reconciliation.merchantCreditsAmount);
  });
});